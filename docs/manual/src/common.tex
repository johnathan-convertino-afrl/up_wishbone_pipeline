...
\par
The only module is the up\_wishbone\_pipeline c module. It is listed below.

\begin{itemize}
  \item \textbf{up\_wishbone\_pipeline} Convert Wishbone Pipeline to the Analog Devices uP BUS. (see core for documentation).
\end{itemize}
...
always:
\begin{itemize}
\item Register Requests
  \begin{enumerate}
    \item When cyc is high and stb isn't register signal to keep previous state.
    \item When both cyc and stb are high register the correct request.
  \end{enumerate}
...
\par
The Wishbone Pipeline core is written in Verilog 2001. They should synthesize in any modern FPGA software. The core comes as a fusesoc packaged core and can be
...
\begin{itemize}
\item \textbf{up\_wishbone\_pipeline} Wishbone Pipeline to uP converter\\
\end{itemize}
